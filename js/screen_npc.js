...
cell.innerHTML = "<div style='font-size:20px;'>" + (row ? "🕓" : "➕") +
"</div><div>" + (SLOT_RU[s] || s) + "</div>" +
...
Router.open("wardrobe", { owner_type: "npc", owner_id: params.id, owner_name: npc.name });
}, "ghost"));
}
box.appendChild(bar);
...
Router.open("ency_edit", { entity: "npc", id: params.id, rec: npc, types: res.types || {} });
}));
})();
});
