/* Меню v2, «🖼 Редактор иконки» (20.09.2026): квадратное окно
предпросмотра + слайдеры zoom (1..5) и dx/dy (-0.5..0.5, доли размера);
предпросмотр математически совпадает с серверным кропом /api/icon/edit
(сторона=min(w,h)/zoom, центр=(w/2+dx*w, h/2+dy*h)). Кнопки:
«🎯 Авто (лицо)» — дефолтные параметры лицевой зоны; «💾 Сохранить иконку»
— серверный кроп и запись (герой → avatar_media_id, NPC → image_media_id).
Источник: герой = fullbody/image; NPC = icon_source_media_id (исходник). */
(function () {
Router.register("face_crop", function (box, params) {
const et = params.entity_type || "hero";
const id = params.id || 0;
let zoom = 2.2, dx = 0, dy = -0.18;
let W = 0, H = 0;
box.appendChild(Router.el("div", "bar",
"<span class='chip'>🖼 Редактор иконки: " +
(et === "hero" ? "герой" : "NPC") + "</span>"));
const win = Router.el("div", "card pad");
win.style.cssText = "position:relative;width:260px;height:260px;" +
"overflow:hidden;border-radius:12px;background:#2d2547;margin:0 auto;";
...
const iZoom = slider("Масштаб (zoom)", 1, 5, 0.05,
function () { return zoom; }, function (v) { zoom = v; });
const iDx = slider("Сдвиг X (доли ширины)", -0.5, 0.5, 0.01,
function () { return dx; }, function (v) { dx = v; });
const iDy = slider("Сдвиг Y (доли высоты)", -0.5, 0.5, 0.01,
...
bar.appendChild(Router.btn("💾 Сохранить иконку", async function () {
