/* Меню v2, «📚 Энциклопедия» v6 (22.09.2026): v5 + кнопка «➕ Создать»
СПРАВА от «🎨 Общая генерация картинок» (требование владельца): открывает
ency_new с entity текущей вкладки (тип создаётся по схеме screen_ency_new v2).
Списки — плитки 2 колонки с «🗑» (подтверждение → /api/ency/delete; сервер
чистит связи; активные мир/герой защищены); СВЕРХУ прогресс генерации
(/api/ency/gen_status, done/total); роутинг: scenario→world_view,
location→location_card, item→item_card, race→race_card, faction→faction_card,
...
const EXTRA_RU = {
  ...
  max_level: "Предел уровня", genre: "Жанр",
  power_grade: "Сила эффекта", duration_type: "Длительность",
  element: "Стихия", faction_type: "Тип фракции"};
function lab(k) {
...
Router.register("ency", function (box) {
  ...
  function drawGen(parent, redraw) {
    ...
    bar.appendChild(btn);
    /* v6: создание записи по типу ТЕКУЩЕЙ вкладки */
    bar.appendChild(Router.btn("➕ Создать", function () {
      if (!ENCY_SEL) return Api.toast("Сначала выберите категорию");
      Router.open("ency_new", { entity: ENCY_SEL, preset: {} });
    }, "ghost"));
